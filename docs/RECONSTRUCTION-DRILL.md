...
☐ Obtain a phoenix playbook for the drill target.
    The playbook is embedded in the phoenix package generated by assemble-phoenix-package.py.
    To generate one ad-hoc from bootstrap-state.json:
    python3 -c "
    import json, sys
    sys.path.insert(0, 'proxmox-bootstrap')
    from phoenix_playbook import build_phoenix_playbook
    state = json.load(open('proxmox-bootstrap/bootstrap-state.json'))
    pb = build_phoenix_playbook(state)
    json.dump(pb, open('reconstruction/phoenix-playbook-latest.json', 'w'), indent=2)
    print('Written to reconstruction/phoenix-playbook-latest.json')
    "
