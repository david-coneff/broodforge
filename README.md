...
- **Forgability** — forging the first node produces a forge package the operator
...
**Recovery wave ordering** — the sequence in which components must be recovered:

```
Wave 0   Physical hardware verification (hardware-profile scan on new hardware)
Wave 1   Proxmox host (network, storage, API) — identity preserved, hardware adapts
Wave 2   Forgejo VM (Git hosting — required before GitOps can run)
Wave 3   k3s control plane
Wave 4   Flux CD bootstrap (reconciles everything below)
Wave 5   Platform services (cert-manager, ingress) + Intelligence layer
...
spawn-package-{cell_id}-{new-hostname}-{ts}.tar.gz
  ...
  │   ├── phase-01-storage.sh    ZFS pool creation from plan (topology-adapted)
  │   ├── phase-02-network.sh    Bridge/bond setup from hardware discovery
  │   ├── phase-03-join-proxmox.sh  pvecm add + cluster membership verification
  │   ├── phase-04-vms.sh        OpenTofu apply for this host's VM set
  │   ├── phase-05-cloudinit.sh  Install Cloud-Init snippets into Proxmox snippet store
  │   ├── phase-06-join-k3s.sh   k3s join (worker, server, or HA promotion)
  │   └── phase-07-verify.sh     Post-spawn validation: cluster health, all VMs running
...
phoenix-package-{cell_id}-{hostname}-{ts}.tar.gz
  ├── phoenix-manifest.json     Node identity snapshot:
  │                              VMIDs, IPs, hostnames, k8s certs, cluster role,
  │                              PBS backup locations, restoration scope
  ├── phoenix-plan.json         Hardware-adapted rebuild plan:
  │                              ZFS topology for new disks, bridge layout for new NICs,
  │                              restoration scope selected, RESTORE vs RECREATE per service
  ├── scripts/
  │   ├── phoenix.sh            Orchestrated entry point
  │   ├── phase-00-scan.sh      Hardware scan on new machine; validates against manifest
  │   ├── phase-01-host.sh      Hostname, /etc/hosts, bridges, ZFS, datastores
  │   ├── phase-02-join.sh      Rejoin Proxmox cluster (same cluster, same node identity)
  │   ├── phase-03-restore.sh   PBS restore of VMs in restoration scope
  │   ├── phase-04-recreate.sh  Recreate VMs flagged RECREATE (IaC + Ansible)
  │   ├── phase-05-k8s.sh       Restore k8s node membership (certs + node name preserved)
  │   ├── phase-06-gitops.sh    Verify Flux reconciliation for restored workloads
  │   └── phase-07-verify.sh    Full health check; update bootstrap-state.json
  ├── identity/
  │   ├── k8s-certs/            Node certificates (from PBS backup or Forgejo)
  │   └── cluster-join-config/  etcd/k3s join config preserving original membership
  ├── opentofu/                 Original tfvars (identity preserved; hardware vars updated)
...
Phases 7.4–12.E per [ROADMAP.md](ROADMAP.md): recovery documentation service layer,
network topology as code, phoenix playbooks, operational documentation, capacity
model, reconstruction drill, and node spawn automation (Phase 12.E — assessment
engine produces a spawn package the broodling runs after bare Proxmox install to
join the hatchery cleanly, with autonomous or interactive execution mode).
