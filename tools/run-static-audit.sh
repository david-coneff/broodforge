...
if [[ -d "${REPO_ROOT}/tests/bash" ]] && ls "${REPO_ROOT}/tests/bash/"*.bats &>/dev/null 2>&1; then
    if command -v bats &>/dev/null; then
        ...
        BATS_PASSED=$(echo "${BATS_OUT}" | { grep -c "^ok " || true; })
        BATS_FAILED=$(echo "${BATS_OUT}" | { grep -c "^not ok " || true; })
...

{
    echo ""
    echo "---"
    echo "_Generated by \`tools/run-static-audit.sh\` (Phase 1.L/1.M, AD-062)_"
} >> "${REPORT}"
